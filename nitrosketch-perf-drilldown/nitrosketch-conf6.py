#!/usr/bin/python
#
# univmon.py eBPF NitroSketch implementation
#
...
def query_sketch(cmd):
    ...
    cs = (CountSketch * cs_table.total_cpu)()
    final_sketch = CountSketch()
    ...
    for i in range(cs_table.total_cpu):
        for row in range(CS_ROWS):
            for column in range(CS_COLUMNS):
                final_sketch.values[row][column] += cs[i].values[row][column] 
    
    sketch_value = queryLayer(pkt, final_sketch)
    print(sketch_value)
...
if __name__ == '__main__':
    ...
    # load BPF program
    b = BPF(src_file=f'{sys.path[0]}/../src/ebpf/nitrosketch/nitrosketch_main.h', cflags=custom_cflags, device=None)
