...
struct sketch_layers {
    struct sketch layers[_NUM_LAYERS];
};

BPF_PERCPU_ARRAY(metadata, struct pkt_md, 1);

BPF_TABLE(EXPERIMENT_MAPTYPE, uint32_t, struct sketch_layers, sketch_map, 1);
...
int xdp_prog1(struct CTXTYPE *ctx) {
    ...
    struct sketch_layers *s = sketch_map.lookup(&zero);
    ...
    #if _USE_ATOMIC == 1
        __sync_fetch_and_add(&s->layers[0].values[i][col], 1);
    #else
        s->layers[0].values[i][col] = rand;
    #endif
    }
