...
struct geo_value geo_table1;
struct geo_value geo_table2;
struct geo_value geo_table3;
struct geo_value geo_table4;
...
void init_geo_array(struct geo_value *array) {
	for (int i = 0; i < GEO_ARRAY_SIZE; i++)
        array->geo_array[i] = ran_geometric(0.1);
}
...
lcore_hello(__rte_unused void *arg)
{
	...
	printf("\nRunning with %u consecutive reads in 4 maps.\n", _NUM_LOOKUPS);
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			for (int i=0; i < nb_rx; i++) {
				...
				// Here I do the processing
					...
					idx = geo_table2.geo_array[idx];
				#endif

				#if _NUM_LOOKUPS > 2
					idx &= (GEO_ARRAY_SIZE - 1);
					idx = geo_table3.geo_array[idx];
				#endif

				#if _NUM_LOOKUPS > 3
					idx &= (GEO_ARRAY_SIZE - 1);
					if (geo_table4.geo_array[idx] == CONTROL_VALUE) {
...
main(int argc, char **argv)
{
	...
	srand(time(NULL)); 
	init_geo_array(&geo_table1);
	init_geo_array(&geo_table2);
	init_geo_array(&geo_table3);
	init_geo_array(&geo_table4);
