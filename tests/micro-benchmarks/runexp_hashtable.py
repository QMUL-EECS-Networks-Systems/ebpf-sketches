...
with open(f'{sys.path[0]}/bpf_progs/my_xdp_prog-hashtable.c','r') as f:
    bpf_src=f.read()
...
def one_run(cflags, maptype, table_size, num_insert, action, run, use_perf=False, running_sec=15):
    ...
    try:
        if use_perf:
            outputprofile=f'outputhtab/exp-{maptype}-ts{table_size}-ni{num_insert}-{action}-run{run}.perf.log'
...
def run_experiment_ht(use_percpu=False, table_size=128, num_insert=128, action='ACTION_HTWRITE', use_perf=False):
    if use_percpu:
        maptype='percpu_hash'
    else:
        maptype='hash'
    assert(action in ['ACTION_HTWRITE','ACTION_HTINC','ACTION_HTREAD'])
    
    outputfn=f'outputhtab/exp-{maptype}-ts{table_size}-ni{num_insert}-{action}.log'
    ...
    if not os.path.exists("outputhtab"):
        os.mkdir("outputhtab")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run experiment to benchmark hash tables')
    parser.add_argument("-p", "--perf", action="store_true", help="If set, run perf to profile the eBPF program")

    args = parser.parse_args()

    for use_percpu in [True, False]:
        for table_size in [2**i for i in range(3, 23+1)]:#8 to 8388608
            ...
            for num_insert in [table_size]:
                for action in ['ACTION_HTWRITE','ACTION_HTINC','ACTION_HTREAD']:
