...
TEST_DIR="results/tests/countminsketch-drop"
IFACE="ens4f0"
TEST_DURATION=30
...
echo "Results for CountMin Sketch eBPF" |& tee -a ${new_dir_name}/output.txt
for i in {1..5}
do
   echo "Starting run ${i}"
   sudo ./scripts/setup_flow_director_single_core.sh ${IFACE}

   result=$(timeout -s SIGINT 120 sudo python3 countmin_sketch.py -i ${IFACE} --read ${TEST_DURATION} -q --count-bytes --count-pkts)
...
if [ -z "${1}" ]; then
   ...
   curl -X POST -H 'Content-type: application/json' --data '{"text":"Test CountMin Sketch Drop done!"}' $1
fi
