...
TEST_DIR="results/tests/nitrosketch-drop-unoptimized"
IFACE="ens4f0"
TEST_DURATION=10
...
do
   ...
   do
      ...
      result=$(timeout -s SIGINT 120 sudo python3 ${SCRIPT_BASE_DIR}/nitrosketch-conf2.py -i ${IFACE} -p ${p} --read ${TEST_DURATION} -s ${rand} -q)
...
if [ -z "${1}" ]; then
   ...
   curl -X POST -H 'Content-type: application/json' --data '{"text":"Test Nitrosketch Drop done!"}' $1
fi
