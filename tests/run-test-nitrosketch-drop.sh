...
TEST_DIR="results/tests/nitrosketch-drop"
IFACE="ens4f0"
TEST_DURATION=10
...
do
   ...
   do
      ...
      result=$(timeout -s SIGINT 120 sudo python3 nitrosketch.py -i ${IFACE} -p ${p} --read ${TEST_DURATION} -s ${rand} -q --count-bytes --count-pkts)
