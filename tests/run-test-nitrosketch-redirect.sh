...
TEST_DIR="results/tests/nitrosketch-redirect"
IFACE="ens4f0"
OUT_IFACE="ens4f1"
TEST_DURATION=10
...
do
   ...
   do
      ...
      timeout -s SIGINT 120 sudo python3 nitrosketch.py -i ${IFACE} -p ${p} --read ${TEST_DURATION} -s ${rand} -q -a REDIRECT -o ${OUT_IFACE} |& tee -a ${new_dir_name}/output.txt
...
if [ -z "$1" ]; then
   ...
   curl -X POST -H 'Content-type: application/json' --data '{"text":"Test Nitrosketch Redirect done!"}' $1
fi