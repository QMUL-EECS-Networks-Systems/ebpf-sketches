...
TEST_DIR="results/perf-drilldown/nitrosketch-univmon"
IFACE="ens4f0"
TEST_DURATION=10
PROBS=( 0.01 0.5 1 )
SEEDS=( 1234 2375 3522 23738 1918 )

SCRIPT_BASE_DIR="nitrosketch-univmon-perf-drilldown"

declare -a arr=("nitrosketch-univmon-conf1.py" 
                "nitrosketch-univmon-conf2.py"
...
do
    ...
    do
        ...
        file_name="exp-nsum-conf${run}-perc-${p}"
        for seed in "${SEEDS[@]}"
        do
...
if [ -z "${1}" ]; then
    ...
    curl -X POST -H 'Content-type: application/json' --data '{"text":"Test Nitrosketch Univmon Drop Performance Drilldown done!"}' $1
fi
